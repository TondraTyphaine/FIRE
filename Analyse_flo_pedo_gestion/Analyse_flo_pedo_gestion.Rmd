...
# Fonction pour calculer une matrice de corrélation mixte
 ->
  data_cor

mixed_correlation <- function(data %>% select(-Sample)) {
  n <- ncol(data %>% select(-Sample))
  cor_matrix <- matrix(NA, n, n)
  colnames(cor_matrix) <- colnames(data %>% select(-Sample))
  rownames(cor_matrix) <- colnames(data %>% select(-Sample))
  
  for (i in 1:n) {
    for (j in 1:n) {
      if (i == j) {
        cor_matrix[i, j] <- 1  # Corrélation parfaite sur la diagonale
      } else {
        x <- data[[i]]
        y <- data[[j]]
        ...
        if (is.numeric(x) && is.numeric(y)) {
          ...
          cor_matrix[i, j] <- cor(as.numeric(x), as.numeric(y), method = "pearson")
        }
      }
    }
  }
...
cor_matrix <- mixed_correlation(data%>% select(-Sample))
...
Les individus 1,2 et 3 (Dm) semblent être influencés par les valeurs élevées du pourcentage de lianescent, de phanérophytes, de chaméphytes, de la fauche tardive, de la conservation de bois mort, de l'élagage et de la classification en zone ZNIEFF pour l'axe 1, et par des valeurs élevées de l'épaisseur moyenne de l'horizon O, du pourcentage d'hélophyte et de l'aménagement de structure de loisir pour l'axe 2.  

Les individus 7, 8 et 9 (La) semblent influencés par les mêmes variables que les individus 1, 2 et 3 pour l'axe 1, et par les valeurs les plus évelées du nombre moyen d'horizon et de l'épaisseur moyenne de l'horizon T3 pour l'axe 2.

Concernant les autres individus, leur regroupement est plus diffus. Les individus 4 (Be) et 11 (Ri) sont plutôt influencés par le pourcentage d'argile, de thérophytes et d'hémichryptophytes pour l'axe 1, et par les mêmes variables que les individus 1,2 et 3 pour l'axe 2.
Les individus 6 et 5 (Be) sont influencés par les mêmes variables que les individus 4 et 11 pour l'axe 1, et par les valeurs moyennes des variables contribuant à l'axe 2.
Les individus 10 et 12 sont influencés par les valeurs moyennes des varibales contribuant à l'axe 1, et par le nombre moyen d'horizon et l'épaisseur moyenne de l'horizon T3 pour l'axe 2.
