...
data_flo %>% 
  select(Site, Quadrat, Espece, N_ind) %>%
  filter(!is.na(Espece)) %>%  # Supprimer les lignes avec Espece manquant
  filter(!is.na(N_ind)) %>%   # Supprimer les lignes avec N_ind manquant
  group_by(Site, Quadrat) %>%
  pivot_wider(names_from = Espece, values_from = N_ind) %>% 
  ungroup() %>%
